...
void makePrediction(int year) {
	...
	for (i=0;i<1000;i++){
		...
		for (ii=0;ii<51;ii++){
			...
			for (iii=0;iii<51;iii++){
				int elodiff = round(eloR*pow(correlations[thisstate][iii],8)*pow(.5+correlations[thisstate][iii]/2,6)*pow(.67+correlations[thisstate][iii]/3,4)*pow(.75+correlations[thisstate][iii]/4,2)*2);
