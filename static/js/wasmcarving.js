...
function displayNow() {
	...
	for (var i=0;i<jsPoints.length;i++){
		if (parseInt(jsPoints[i].county)>maxP){
			maxP = parseInt(jsPoints[i].county);
			//console.log(maxP,jsPoints[i].x,jsPoints[i].y);
		}
		if (parseInt(jsPoints[i].county)<minP){
			minP = parseInt(jsPoints[i].county);
		}
		sumP += parseInt(jsPoints[i].county);
	}
	var avgP = sumP/jsPoints.length;
	console.log(minP,maxP, avgP);
	for (var i=0;i<jsPoints.length;i++){
		svg += '<circle r=".5" cx="'+jsPoints[i].x+'" cy="'+jsPoints[i].y+'" fill="rgb('+(parseInt(jsPoints[i].county)*125/avgP)+',0,'+(250-parseInt(jsPoints[i].county)*125/avgP)+')" stroke="none"></circle>';
	}
	svg += '</svg>';
	postMessage(svg);
}
